...
use cc_switch_core::proxy::providers::copilot_auth::{
    CopilotAuthManager, CopilotAuthStatus, CopilotModel, CopilotUsageResponse, GitHubAccount,
    GitHubDeviceCodeResponse,
};
use tauri::State;

/// Copilot 认证状态（类型别名，实际定义在 cc-switch-core）
pub use cc_switch_core::copilot_auth::CopilotAuthState;
...
pub async fn copilot_start_device_flow(
    ...
    let auth_manager = state.inner().read().await;
    auth_manager
...
pub async fn copilot_poll_for_auth(
    ...
    let auth_manager = state.inner().write().await;
    match auth_manager
        ...
        .await
    {
        ...
        Ok(None) => Ok(false),
        Err(
            cc_switch_core::proxy::providers::copilot_auth::CopilotAuthError::AuthorizationPending,
        ) => Ok(false),
        Err(e) => {
