...
>
> 💡 プロバイダー編集中、詳細設定で API 形式を選択できます。以下のプロトコルに対応しています：

| 形式 | 説明 |
|------|------|
| `gemini_native` | Gemini Native generateContent（ネイティブ） |
| `openai_chat` | OpenAI Chat Completions（自動変換） |
| `openai_responses` | OpenAI Responses API（自動変換） |
| `anthropic` | Anthropic Messages API（自動変換） |
