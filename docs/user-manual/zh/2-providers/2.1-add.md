...
>
> 💡 在编辑供应商时，你可以在高级设置中选择 API 格式，支持以下协议：

| 格式 | 说明 |
|------|------|
| `gemini_native` | Gemini Native generateContent（原生） |
| `openai_chat` | OpenAI Chat Completions（自动转换格式） |
| `openai_responses` | OpenAI Responses API（自动转换格式） |
| `anthropic` | Anthropic Messages API（自动转换格式） |
