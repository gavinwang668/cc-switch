...
>
> 💡 When editing a provider, you can select the API format in the advanced settings. The following protocols are supported:

| Format | Description |
|--------|-------------|
| `gemini_native` | Gemini Native generateContent (native) |
| `openai_chat` | OpenAI Chat Completions (auto-converted) |
| `openai_responses` | OpenAI Responses API (auto-converted) |
| `anthropic` | Anthropic Messages API (auto-converted) |
