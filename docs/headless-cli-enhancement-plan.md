# CC Switch 无 GUI 后台运行与 CLI 全面替代 GUI 改造计划

> 文档版本：v1.0  ｜  日期：2026-06-19  ｜  状态：待用户审阅
>
> 目标：评估当前项目在无 GUI 的 Linux 服务器上后台运行、并通过命令行完全替代 GUI 全部配置功能的可行性，给出分阶段改造计划。
...
**结论**：完全替代 GUI 在技术上可行但工作量中等偏大，建议分 4 个阶段渐进改造。约 70% 的功能可低成本复用现有服务层，15% 需中等改造，15% 属于"GUI 专属"不建议或无必要在 CLI 中实现。
